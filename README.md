# 🏦 Hệ thống Phân tích & Quản trị Rủi ro Ngân hàng (SQL & Power BI Analytics)

## 📌 Tổng quan Dự án
Dự án ứng dụng SQL Server (T-SQL) để xử lý cơ sở dữ liệu hệ thống ngân hàng bán lẻ, kết hợp với Power BI nhằm thiết lập các mô hình tầm soát, đo lường và trực quan hóa cảnh báo sớm các danh mục rủi ro cốt lõi bao gồm: **Rủi ro Tín dụng (Credit Risk)**, **Rủi ro Gian lận (Fraud Risk)**, và **Rủi ro Vận hành (Operational Risk)**.

---

## 🖥️ Executive Dashboard (Tổng quan Báo cáo)
...
### 🎨 2. Trực quan hóa & Phân tích Động (Front-End)
* **Hệ thống Business Intelligence:** Power BI Desktop (File đính kèm: `Banking.pbix`).
* **Kỹ thuật Power BI áp dụng:**
  - **Data Modeling (Mô hình hóa dữ liệu):** Thiết kế mô hình Star Schema tối ưu hóa các mối quan hệ giữa bảng Fact (`banking transactions`) và các bảng Dimension (`users`, `cards`, `mcc_codes`).
  - **Định hướng bộ lọc (Cross-filtering):** Cấu hình bộ lọc hai chiều (Both-directional filtering) giải quyết triệt để lỗi đồng bộ dữ liệu, giúp truyền bộ lọc chính xác từ danh mục ngành hàng sang các chỉ số rủi ro.
  - **Ngôn ngữ DAX:** Xây dựng hệ thống Measure tính toán động thời gian thực (`% High Credit Risk`, `Op Loss Count`, `Op Loss Pct`).
  - **Tư duy UI/UX Dashboard:** Triển khai thiết kế theo cấu trúc Grid Layout trên nền tối (Dark Theme), kết hợp phân cấp thông tin rõ ràng từ KPI tổng quan đến các biểu đồ chuyên sâu (Trục kép, Treemap, Top 10 danh mục nguy cơ).
